---
title: "Shared Tweets: An Analysis of Global Twitter Data"
author: "Jasper Palmer, Toan Ly, Ruizhe Zhou, and Nhat Le"
date: "2/28/2020"
output: html_document
---

```{r echo = F, message = F}
# russian_troll_df <- read.csv("data/IRAhandle_tweets_1.csv")
library("knitr")
source("scripts/summary-paragraph.R")
source("scripts/summary-table.R")
source("scripts/first-chart.R")
source("scripts/second-chart.R")
source("scripts/third-chart.R")
# sentiment_df <- read.csv("data/training.1600000.processed.noemoticon.csv", check.names = F, stringsAsFactors = F)
# colnames(sentiment_df) <- c("target", "id", "date", "flag", "user", "text")
senators_df <- read.csv("data/senators.csv", stringsAsFactors = F, check.names = F)

```

<style>
.html-widget {
    margin: auto;
    margin-top: 25px;
    margin-bottom: 25px;
}
</style>

# Introduction

The domain that we are focusing on is Twitter data as it is a highly popular social media platform that gives us an insight to individuals mindset through short text summaries called tweets. These individuals include influential public figures such as senators, as well as, any person that chooses to use the platform. This gives us data on social media usage in a standardized format that will allow us to draw conclusions about a wide variety of individuals. Twitter is also an optimal choice because it is a primarily text based allowing us to analyze information rather than missing a whole subsection of context such as Instagram’s focus on pictures.

# Summary Paragraph



# Summary Table

```{r, echo = F} 
#Add function call here
```

# Does a Tweet's Hashtags Affect its Retweets?

These charts were intended to show the relationship between the number of hashtags taht a tweet receives and the number of retweets that it gets. The two charts are also meant to compare the relationship between official figures (US senators) and unofficial troll tweets (essentially spam). 

```{r, echo = F, message = F} 
hashtag_vs_retweet(senators_df)
```
As we can see from the first chart, the overall trend (excluding the outlier of eight hashtags) is a significant decline in average number of retweets per tweet for each hashtag used a U.S. Senator's post. This trend is not consistent with the number of retweets for the 8 hashtag tweets, however, there were only six tweets with eight hashtags and one tweet happened to have 2516 retweets which heavily scewed its average result.

# [Chart Name 2]

```{r, echo = F} 
#Add function call here
```

# [Chart Name 3]

```{r, echo = F} 
#Add function call here
```




